...
def test():
	...
	batch_size = 1
	datasize, datasize_trained = 540, 300
	...
	logging.info('training datasize: {}'.format(datasize_trained))

	with tf.Session() as sess:

		accuracy_, accuracy_3_, loss_, normal_, BRDF_, I_, lr_, lp_, dif_ = model.net('predicting')
		...
		for i in range(datasize):
			if normal_ori_i == 0:
				accuracy, accuracy_3, loss, lr, lp, pre_normal, BRDF, img, dif = sess.run(
					[accuracy_, accuracy_3_, loss_, lr_, lp_, normal_, BRDF_, I_, dif_], 
					feed_dict = {
			...
			logging.info("{}: data {}, \taccuracy {}, \t{}, \tloss {}".format(
